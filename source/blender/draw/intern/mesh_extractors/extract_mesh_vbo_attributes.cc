...
static void extract_mesh_attr_overlay_init(const MeshRenderData *mr,
...
{
  MeshExtract extractor = {nullptr};
  extractor.init = extract_mesh_attr_overlay_init;
  extractor.data_type = MR_DATA_NONE;
  extractor.data_size = 0;
  extractor.use_threading = false;
  extractor.mesh_buffer_offset = offsetof(MeshBufferList, vbo.attr_overlay);
  return extractor;
}
...
const MeshExtract extract_attr_overlay = blender::draw::create_extractor_attr_overlay();
