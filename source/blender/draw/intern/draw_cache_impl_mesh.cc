  ...
  switch (buffer_index) {
    case BUFFER_INDEX(vbo.pos_nor):
      return MBC_SURFACE | MBC_SURFACE_WEIGHTS | MBC_EDIT_TRIANGLES | MBC_EDIT_VERTICES |
             ...
             MBC_WIRE_EDGES | MBC_WIRE_LOOPS | MBC_SCULPT_OVERLAYS | MBC_ATTRIBUTE_OVERLAY |
             MBC_SURFACE_PER_MAT;
    ...
    case BUFFER_INDEX(vbo.attr_overlay):
      return MBC_ATTRIBUTE_OVERLAY;
...
GPUBatch *DRW_mesh_batch_cache_get_surface_attribute(Mesh *me)
{
  MeshBatchCache *cache = mesh_batch_cache_get(me);

  mesh_batch_cache_add_request(cache, MBC_ATTRIBUTE_OVERLAY);
  DRW_batch_request(&cache->batch.attribute_overlay);

  return cache->batch.attribute_overlay;
}
                                           ...
                                           const bool use_hide)
{
  ...
  assert_deps_valid(
      MBC_ATTRIBUTE_OVERLAY,
      {BUFFER_INDEX(ibo.tris), BUFFER_INDEX(vbo.pos_nor), BUFFER_INDEX(vbo.attr_overlay)});
  if (DRW_batch_requested(cache->batch.attribute_overlay, GPU_PRIM_TRIS)) {
    DRW_ibo_request(cache->batch.attribute_overlay, &mbuflist->ibo.tris);
    DRW_vbo_request(cache->batch.attribute_overlay, &mbuflist->vbo.pos_nor);
    DRW_vbo_request(cache->batch.attribute_overlay, &mbuflist->vbo.attr_overlay);
  }
  ...
  assert_final_deps_valid(BUFFER_INDEX(vbo.attr_overlay));
