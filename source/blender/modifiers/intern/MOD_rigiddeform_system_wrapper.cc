...
RigidDeformSystemRef RigidDeformSystem_from_mesh(Mesh *mesh)
{
  return wrap(new RigidDeformSystem(get_vertex_positions(mesh), get_triangle_indices(mesh)));
}

void RigidDeformSystem_set_anchors(RigidDeformSystemRef system,
                                   uint *anchor_indices,
                                   uint anchor_amount)
{
...
void RigidDeformSystem_correct_inner(RigidDeformSystemRef system,
                                     Vector3Ds positions,
                                     uint iterations)
{
  if (iterations == 0) {
...
void RigidDeformSystem_free(RigidDeformSystemRef system)
{
