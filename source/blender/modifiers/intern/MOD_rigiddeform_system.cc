...
static std::vector<double> calc_total_weight_per_vertex(const WeightedEdges &edges,
                                                        uint vertex_amount)
{
...
static std::array<double, 3> triangle_corner_angles(Eigen::Vector3d v1,
                                                    Eigen::Vector3d v2,
                                                    Eigen::Vector3d v3)
{
...
static WeightedEdges calculate_cotan_edge_weights(
    const Vectors &positions, const std::vector<std::array<uint, 3>> &triangles)
{
  WeightedEdges edges;
  double eps = 0.0001;
  ...
  for (auto verts : triangles) {
    ...
    if (w1 > eps)
      edges.push_back(WeightedEdge(verts[1], verts[2], w1));
    if (w2 > eps)
      edges.push_back(WeightedEdge(verts[0], verts[2], w2));
    if (w3 > eps)
      edges.push_back(WeightedEdge(verts[0], verts[1], w3));
  }
...
static Triplets get_laplace_matrix_triplets(uint vertex_amount, const WeightedEdges &edges)
{
...
RigidDeformSystem::RigidDeformSystem(const Vectors &initial_positions,
...
void RigidDeformSystem::set_anchors(const std::vector<uint> &anchor_indices)
{
  ...
  for (Triplet triplet : m_laplace_triplets) {
    if (m_order.is_inner__orig(triplet.row())) {
      ...
      if (m_order.is_inner__orig(triplet.col())) {
        triplets_A_II.push_back(
            Triplet(reorder_row, m_order.to_new(triplet.col()), triplet.value()));
      }
      else {
        triplets_A_IB.push_back(
            Triplet(reorder_row, m_order.to_new_anchor(triplet.col()), triplet.value()));
      }
    }
  }
...
void RigidDeformSystem::get_impact_edges(WeightedEdges &r_impact_edges,
...
Vectors RigidDeformSystem::calculate_inner(const Vectors &anchor_positions, uint iterations)
{
  assert(iterations > 0);
...
static inline Eigen::Vector3d get_position(uint index,
...
std::vector<Eigen::Matrix3d> RigidDeformSystem::optimize_rotations(
    const Vectors &anchor_positions, const Vectors &new_inner_positions)
{
  ...
  for (WeightedEdge edge : m_impact.edges()) {
    ...
    Eigen::Vector3d edge_new_start = get_position(
        v1, m_order, anchor_positions, new_inner_positions);
    Eigen::Vector3d edge_new_end = get_position(
        v2, m_order, anchor_positions, new_inner_positions);
...
Vectors RigidDeformSystem::optimize_inner_positions(const AnchorData &anchor_data,
...
Vectors RigidDeformSystem::calculate_new_inner_diffs(const std::vector<Eigen::Matrix3d> &rotations)
{
  ...
  for (WeightedEdge edge : m_impact.edges()) {
    ...
    Eigen::Vector3d value = edge.weight / 2.0f * (rotations[v1_compact] + rotations[v2_compact]) *
                            old_edge;
...
Vectors RigidDeformSystem::solve_for_new_inner_positions(const AnchorData &anchor_data,
 ...
 * (6, [1, 4]) -> [0, 2, 3, 5,  1, 4] */
static std::vector<uint> sort_vertices_by_anchors(const std::vector<uint> &anchors,
                                                  uint vertex_amount)
{
