...
static void get_all_vertex_weights(Object *ob, Mesh *mesh, const char *name, float *dst)
{
  ...
  for (int i = 0; i < mesh->totvert; i++) {
    dst[i] = BKE_defvert_find_weight(vertices + i, group_index);
  }
}
...
static void get_non_zero_weight_indices(
    Object *ob, Mesh *mesh, const char *weight_group_name, int **r_indices, int *r_amount)
{
...
static void get_anchor_indices(
    Object *ob, Mesh *mesh, const char *anchor_group_name, int **r_indices, int *r_amount)
{
  get_non_zero_weight_indices(ob, mesh, anchor_group_name, r_indices, r_amount);
}

static BindData *bind_data_calculate(RigidDeformModifierData *rdmd,
                                     Object *ob,
                                     Mesh *mesh,
                                     VectorArray vertex_cos)
{
  ...
  if (!vertex_group_exists(ob, mesh, rdmd->anchor_group_name)) {
    modifier_setError(
        &rdmd->modifier, "Vertex group '%s' does not exist.", rdmd->anchor_group_name);
    return NULL;
  }
  ...
  get_anchor_indices(
      ob, mesh, rdmd->anchor_group_name, &bind_data->anchor_indices, &bind_data->anchor_amount);
...
static void bind_current_mesh_to_modifier(RigidDeformModifierData *rdmd,
...
static void update_bound_anchors(RigidDeformModifierData *rdmd, Object *ob, Mesh *mesh)
{
  ...
  if (rdmd->cache != NULL) {
    RigidDeformSystem_set_anchors(((Cache *)rdmd->cache)->system,
...
static void deform_vertices(RigidDeformModifierData *rdmd, Mesh *mesh, VectorArray vertex_cos)
{
  ...
  if (cache->system == NULL) {
    ...
    RigidDeformSystem_set_anchors(cache->system,
                                  (uint *)rdmd->bind_data->anchor_indices,
                                  (uint)rdmd->bind_data->anchor_amount);
  }
...
static RigidDeformModifierData *get_original_modifier_data(RigidDeformModifierData *rdmd,
...
static void run_modifier(RigidDeformModifierData *md,
                         const ModifierEvalContext *ctx,
                         Mesh *mesh,
                         VectorArray vertex_cos)
{
                        ...
                        int vertex_amount)
{
  Mesh *mesh_src = MOD_deform_mesh_eval_get(
      ctx->object, NULL, mesh, NULL, vertex_amount, false, false);
...
{
  Mesh *mesh_src = MOD_deform_mesh_eval_get(
      ctx->object, editData, mesh, NULL, vertex_amount, false, false);
