...
static void add_mapping_information(VTreeMultiFunctionMappings &mappings)
{
  add_basic_type<float>(mappings, "Float", INSERT_float_socket);
  add_basic_type<float3>(mappings, "Vector", INSERT_vector_socket);
...
BLI_LAZY_INIT_REF(const VTreeMultiFunctionMappings, get_vtree_multi_function_mappings)
{
  ...
  add_mapping_information(*mappings);
  return mappings;
}
