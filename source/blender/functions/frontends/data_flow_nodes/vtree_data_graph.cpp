...
Vector<const VSocket *> VTreeDataGraph::find_placeholder_dependencies(
    ArrayRef<const VSocket *> vsockets)
{
...
Vector<const VSocket *> VTreeDataGraph::find_placeholder_dependencies(ArrayRef<DataSocket> sockets)
{
  Stack<DataSocket> to_be_checked = sockets;
  Set<DataSocket> found = sockets;
  Vector<const VSocket *> vsocket_dependencies;
  ...
  while (!to_be_checked.empty()) {
    ...
    if (socket.is_input()) {
      ...
      if (fn.has_body<VNodePlaceholderBody>()) {
        ...
        const VNode &vnode = body.vnode();
        uint data_output_index = m_graph->index_of_output(socket);
        const VSocket &vsocket = this->find_data_output(vnode, data_output_index);
...
const VSocket &VTreeDataGraph::find_data_output(const VNode &vnode, uint index)
{
  uint count = 0;
  for (uint i = 0; i < vnode.outputs().size(); i++) {
    const VSocket &vsocket = vnode.output(i);
