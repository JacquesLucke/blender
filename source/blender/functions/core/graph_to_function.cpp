...
namespace FN {
	...
	public:
		...
		{
			for (uint i = 0; i < this->m_outputs.size(); i++) {
				this->compute_socket(fn_in, fn_out, i, this->m_outputs[i]);
			}
		}

		void compute_socket(const Tuple &fn_in, Tuple &out, uint out_index, Socket socket) const
		{
			if (this->m_inputs.contains(socket)) {
				uint index = this->m_inputs.index(socket);
