...
class DerivedNodeTreeSGraphAdapter {
 private:
 ...
 public:
  ...
  DerivedNodeTreeSGraphAdapter(const DerivedNodeTree &tree) : tree_(tree)
  {
  }

  int node_inputs_size(const DNode &node) const
  {
    return node->inputs().size();
  }

  int node_outputs_size(const DNode &node) const
  {
  ...
  {
    tree_.foreach_node(f);
  }

  template<typename F>
  void foreach_linked_input(const DNode &node, const int output_socket_index, const F &f) const
  {
    const DOutputSocket output_socket = node.output(output_socket_index);
    output_socket.foreach_target_socket(
        [&](const DInputSocket input_socket,
          ...
          f(input_socket.node(), input_socket->index());
        });
  }

  template<typename F>
  void foreach_linked_output(const DNode &node, const int input_socket_index, const F &f) const
  {
    const DInputSocket input_socket = node.input(input_socket_index);
    input_socket.foreach_origin_socket([&](const DSocket &origin_socket) {
      if (origin_socket->is_input()) {
        return;
      }
      f(origin_socket.node(), origin_socket->index());
    });
  }

  std::string node_debug_name(const DNode &node) const
  {
    return node->name();
  }

  std::string input_socket_debug_name(const DNode &node, const int input_socket_index) const
  {
    return node->input(input_socket_index).name();
  }

  std::string output_socket_debug_name(const DNode &node, const int output_socket_index) const
  {
