...
class NodeTreeRefLogicalAdapter : public fn::NodeGraphAdapterBase<NodeTreeRefLogicalAdapter> {
 private:
 ...
 public:
  ...
  NodeTreeRefLogicalAdapter(const NodeTreeRef &tree) : tree_(tree)
  {
  }
  ...
  template<typename F> void foreach_node(const F &f) const
  {
    for (const NodeRef *node : tree_.nodes()) {
      if (!node->is_muted() && !node->is_frame() && !node->is_reroute_node()) {
        f(node);
      }
    }
  }
  ...
  void foreach_linked_input(const NodeRef *node, const int output_socket_index, const F &f) const
  {
    ...
    for (const InputSocketRef *linked_socket : socket.logically_linked_sockets()) {
  ...
  void foreach_linked_output(const NodeRef *node, const int input_socket_index, const F &f) const
  {
    ...
    for (const OutputSocketRef *linked_socket : socket.logically_linked_sockets()) {
      f(&linked_socket->node(), linked_socket->index());
    }
  }
