...
static void geo_node_point_separate_exec(GeoNodeExecParams params)
{
  ...
  wait_for_inputs |= params.require_input_if_not_available("Mask");
  wait_for_inputs |= params.require_input_if_not_available("Geometry");
  if (wait_for_inputs) {
    return;
  }
