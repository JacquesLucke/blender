 ...
 */
static void cast_elem(const eSDNA_Type old_type,
 ...
 *
 * \param reconstruct_info: Preprocessed reconstruction information generated by
 * #DNA_reconstruct_info_new.
 * \param new_struct_number: Index in newsdna->structs of the struct that is being reconstructed.
...
static void reconstruct_struct(const DNA_ReconstructInfo *reconstruct_info,
                               const int new_struct_number,
                               const char *old_block,
                               char *new_block)
{
  const ReconstructStep *steps = reconstruct_info->steps[new_struct_number];
  const int step_count = reconstruct_info->step_counts[new_struct_number];
  ...
  for (int a = 0; a < step_count; a++) {
    ...
    switch (step->type) {
      ...
      case RECONSTRUCT_STEP_CAST_ELEMENT:
        cast_elem(step->data.cast_element.old_type,
                  step->data.cast_element.new_type,
                  step->data.cast_element.amount,
                  old_block + step->data.cast_element.old_offset,
                  new_block + step->data.cast_element.new_offset);
        break;
      case RECONSTRUCT_STEP_CAST_POINTER_TO_32:
        cast_pointer_64_to_32(step->data.cast_pointer.amount,
      ...
      case RECONSTRUCT_STEP_CAST_POINTER_TO_64:
        cast_pointer_32_to_64(step->data.cast_pointer.amount,
      ...
      case RECONSTRUCT_STEP_SUBSTRUCT:
        reconstruct_struct(reconstruct_info,
                           step->data.substruct.new_struct_number,
                           old_block + step->data.substruct.old_offset,
                           new_block + step->data.substruct.new_offset);
        break;
      case RECONSTRUCT_STEP_INIT_ZERO:
        // BLI_assert(!"should have been removed during step compression");
        break;
    }
  }
}
...
/**
 * \param reconstruct_info: Information preprocessed by #DNA_reconstruct_info_new.
 * \param old_struct_number: Index of struct info within oldsdna.
                             ...
                             const void *old_blocks)
{
  ...
  const int new_struct_number = DNA_struct_find_nr(newsdna, type_name);

  if (new_struct_number == -1) {
    return NULL;
  }

  const SDNA_Struct *new_struct = newsdna->structs[new_struct_number];
  const int old_block_size = oldsdna->types_size[old_struct->type];
  const int new_block_size = newsdna->types_size[new_struct->type];
  BLI_assert(new_block_size > 0); /* Is this safe to assume? */

  char *new_blocks = MEM_callocN(blocks * new_block_size, "reconstruct");
  for (int a = 0; a < blocks; a++) {
    const char *current_old_block = POINTER_OFFSET(old_blocks, a * old_block_size);
    char *current_new_block = POINTER_OFFSET(new_blocks, a * new_block_size);
    reconstruct_struct(reconstruct_info, new_struct_number, current_old_block, current_new_block);
  }
  ...
  switch (new_category) {
    case STRUCT_MEMBER_CATEGORY_STRUCT: {
      if (STREQ(new_type_name, old_type_name)) {
        const int old_struct_number = DNA_struct_find_nr(oldsdna, old_type_name);
        BLI_assert(old_struct_number != -1);
        enum eSDNA_StructCompare compare_flag = compare_flags[old_struct_number];
        ...
        if (compare_flag == SDNA_CMP_EQUAL) {
          ...
          const int new_struct_number = DNA_struct_find_nr(newsdna, new_type_name);
          BLI_assert(new_struct_number != -1);
          ...
          r_step->data.substruct.amount = shared_array_length;
          r_step->data.substruct.new_struct_number = new_struct_number;
        }
      }
      else {
    ...
    case STRUCT_MEMBER_CATEGORY_PRIMITIVE: {
      if (STREQ(new_type_name, old_type_name)) {
        ...
        r_step->type = RECONSTRUCT_STEP_CAST_ELEMENT;
        r_step->data.cast_element.amount = shared_array_length;
        r_step->data.cast_element.new_offset = new_member_offset;
        r_step->data.cast_element.old_offset = old_member_offset;
        r_step->data.cast_element.new_type = new_member->type;
        r_step->data.cast_element.old_type = old_member->type;
      }
      break;
    }
    case STRUCT_MEMBER_CATEGORY_POINTER: {
      if (newsdna->pointer_size == oldsdna->pointer_size) {
        ...
        r_step->data.cast_pointer.amount = shared_array_length;
      }
  ...
  switch (step->type) {
    ...
    case RECONSTRUCT_STEP_CAST_ELEMENT: {
      printf(
          ...
          "%d, amount: %d",
          (int)step->data.cast_element.old_type,
          oldsdna->types[step->data.cast_element.old_type],
          (int)step->data.cast_element.new_type,
          newsdna->types[step->data.cast_element.new_type],
          step->data.cast_element.old_offset,
          step->data.cast_element.new_offset,
          step->data.cast_element.amount);
      break;
    }
    case RECONSTRUCT_STEP_CAST_POINTER_TO_32: {
      printf("pointer to 32, old offset: %d, new offset: %d, amount: %d",
             step->data.cast_pointer.old_offset,
             step->data.cast_pointer.new_offset,
             step->data.cast_pointer.amount);
      break;
    }
    case RECONSTRUCT_STEP_CAST_POINTER_TO_64: {
      printf("pointer to 64, old offset: %d, new offset: %d, amount: %d",
             step->data.cast_pointer.old_offset,
             step->data.cast_pointer.new_offset,
             step->data.cast_pointer.amount);
      break;
    ...
    case RECONSTRUCT_STEP_SUBSTRUCT: {
      printf(
          ...
          step->data.substruct.new_struct_number,
          newsdna->types[newsdna->structs[step->data.substruct.new_struct_number]->type],
          newsdna->types_size[newsdna->structs[step->data.substruct.new_struct_number]->type],
          step->data.substruct.amount);
      break;
    }
  }
}
                                              ...
                                              const char *compare_flags)
{
  ...
  for (int new_struct_number = 0; new_struct_number < newsdna->structs_len; new_struct_number++) {
    ...
    reconstruct_info->step_counts[new_struct_number] = steps_len;

/* This is useful when debugging the reconstruction steps. */
#if 0
    printf("%s: \n", new_struct_name);
