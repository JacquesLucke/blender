...
TEST(virtual_array, Span)
{
  ...
  VArrayImpl_For_Span<int> varray{data};
...
{
  VArrayImpl_For_Single<int> varray{10, 4};
...
TEST(virtual_array, Array)
{
  ...
  {
    VArrayImpl_For_ArrayContainer varray{array};
    EXPECT_EQ(varray.size(), 5);
  ...
  {
    VArrayImpl_For_ArrayContainer varray{std::move(array)};
    EXPECT_EQ(varray.size(), 5);
    EXPECT_EQ(varray[0], 1);
    EXPECT_EQ(varray[2], 3);
    EXPECT_EQ(varray[3], 5);
  ...
  {
    VArrayImpl_For_ArrayContainer varray{array}; /* NOLINT: bugprone-use-after-move */
...
TEST(virtual_array, Vector)
{
  ...
  VArrayImpl_For_ArrayContainer varray{std::move(vector)};
  EXPECT_EQ(varray.size(), 4);
...
TEST(virtual_array, VectorSet)
{
  ...
  VArrayImpl_For_ArrayContainer varray{std::move(vector_set)};
...
TEST(virtual_array, Func)
{
  ...
  VArrayImpl_For_Func<int, decltype(func)> varray{10, func};
...
TEST(virtual_array, DerivedSpan)
{
  ...
  vector.append({1, 1, 1});
  {
    VArrayImpl_For_DerivedSpan<std::array<int, 3>, int, get_x> varray{vector};
    EXPECT_EQ(varray.size(), 2);
    EXPECT_EQ(varray[0], 3);
    EXPECT_EQ(varray[1], 1);
  }
  {
    VMutableArrayImpl_For_DerivedSpan<std::array<int, 3>, int, get_x, set_x> varray{vector};
    EXPECT_EQ(varray.size(), 2);
    EXPECT_EQ(varray[0], 3);
    EXPECT_EQ(varray[1], 1);
    varray.set(0, 10);
