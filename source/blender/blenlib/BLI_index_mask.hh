 ...
 * range. The latter one is more efficient due to less memory reads.
 *
 * The IndexMask.foreach_index method helps writing code that implements both code paths at the
 * same time. However, sometimes the additional knowledge can be used to use more efficient
 * algorithms or SIMD instructions. Note: Be careful when nesting multiple calls to
 * IndexMask.foreach_index, it might lead to a lot of generated code due to a combinatorial
 * explosion.
 */
...
class IndexMask {
 ...
 public:
  ...
  /**
   * Get the n-th index referenced by this IndexMask. The `index_mask` method returns an IndexRange
   * containing all indices that can be used as parameter here.
   */
  ...
  /**
   * Get the IndexRange referenced by this IndexMask. This method should only be called after the
   * caller made sure that this IndexMask is actually a range.
   */
  ...
  /**
   * Get an IndexRange that can be used to index this IndexMask.
   *
  ...
  /**
   * Get the largest index that is referenced by this index mask.
   */
  uint last() const
  {
